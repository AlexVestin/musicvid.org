...
export default class NoiseMaterial extends ShaderToyMaterial {
    constructor(item) {
        ...
        loadImageTextureFromChoice(
            "./img/noise/" + this.prevFile,
            this.setTexture
        );
    ...
    __setUpGUI = (folder) => {
        ...
        i.addController(folder, this, "width").onChange(
            () =>
        ...
        i.addController(folder, this, "height").onChange(
            () =>
                (this.uniforms.iResolution.value = new THREE.Vector2(
                    this.width,
                    this.height
        ...
        i.addController(folder, this.uniforms.textureZoom, "value", {
            min: 0,
            max: 40,
            path: "TextureZoom"
        }).name("Texture zoom");
        i.addController(folder, this.uniforms.red, "value", {
            min: 0,
            max: 1,
            step: 0.01,
            path: "red"
        }).name("Red");
        i.addController(folder, this.uniforms.green, "value", 0, 1, 0.01).name(
            "Green"
        );
        i.addController(folder, this.uniforms.blue, "value", 0, 1, 0.01).name(
            "Blue"
        );
    ...
    updateMaterial = (time, dt, audioData) => {
        ...
        if (this.impactAnalyser) {
            ...
            this.time +=
                this.baseSpeed * 0.01 + (dt * impact * this.amplitude) / 10;
